...
class Auth
{
    ...
    public function checkOTP($username)
    {
    ...
    {
        $userdata = $this->getUserData($username);
        $query = $this->dbh->prepare("SELECT secret FROM {$this->config->table_otp} WHERE uid = ?");
    ...
    protected function _base32Decode($secret)
    {
        ...
        if (!in_array($paddingCharCount, $allowedValues))
            return false;
        for ($i = 0; $i < 4; $i++) {
            if ($paddingCharCount == $allowedValues[$i] && substr($secret, -($allowedValues[$i])) !=
        ...
        for ($i = 0; $i < count($secret); $i = $i + 8) {
            ...
            if (!in_array($secret[$i], $base32chars))
                return false;
            for ($j = 0; $j < 8; $j++) {
                $x .= str_pad(base_convert(@$base32charsFlipped[@$secret[$i + $j]], 10, 2), 5,
                    '0', STR_PAD_LEFT);
            }
            $eightBits = str_split($x, 8);
            for ($z = 0; $z < count($eightBits); $z++) {
                $binaryString .= (($y = chr(base_convert($eightBits[$z], 2, 10))) || ord($y) ==
                    48) ? $y : "";
            }
        }
        return $binaryString;
    ...
    protected function _base32Encode($secret, $padding = true)
    {
        ...
        for ($i = 0; $i < count($secret); $i++) {
            $binaryString .= str_pad(base_convert(ord($secret[$i]), 10, 2), 8, '0',
                STR_PAD_LEFT);
        }
        ...
        while ($i < count($fiveBitBinaryArray)) {
            $base32 .= $base32chars[base_convert(str_pad($fiveBitBinaryArray[$i], 5, '0'), 2,
                10)];
            $i++;
        }
